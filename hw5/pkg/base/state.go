...
func (s *State) HandleMessage(index int, deleteMessage bool) (result []*State) {
	//TODO: implement it
	to := s.Network[index].To()
	node_receiver := s.nodes[to]
	newNodes := node_receiver.MessageHandler(s.Network[index])
	// fmt.Printf("new nodes: %v, message: %v\n", newNodes, s.Network[index])
...
func (s *State) NextStates() []*State {
	...
	for i := range s.Network {
		...
		if s.isDropOff {
			ns := s.Inherit(DropOffEvent(s.Network[i]))
			ns.DeleteMessage(i)
		...
		// (use HandleMessage)
		if s.isDuplicate {
			nStates := s.HandleMessage(i, false)
			nextStates = append(nextStates, nStates...)
		}
...
func (s *State) TriggerNodeTimer(address Address, node Node) []*State {
	...
	for _, n := range newNodes {
		newState := s.Inherit(TriggerEvent(address, n.NextTimer()))
...
func (s *State) nodeAddress(node Node) Address {
	for address, n := range s.nodes {
		if n.Equals(node) {
			return address
		}
	}
...
func (s *State) generateAddress() Address {
	notUnique := true
	addr := ""
	Max := 10000
	for notUnique {
		addr = strconv.Itoa(rand.Intn(Max))
		notUnique = false
